...
function dePalabrasAFrase(palabras) {
  ...
  //return frase;
  const retorno = frase.substring(0, frase.length - 1);//quitar ultimo espacio
  return retorno
...
function promedioResultadosTest(resultadosTest) {
  ...
  var promedio = cont / resultadosTest.length;
  return promedio;
}
...
for(let j=0; j <= buscar.length -1; j++){
  if(buscar[j] === 'Marzo'){
    retornar[0] = buscar[j]
  }else if(buscar[j] === 'Noviembre'){
    retornar[1] = buscar[j]
  }else if(buscar[j] === 'Enero'){
    retornar[2] = buscar[j];
  }
}

if(retornar[0] === 'Marzo' && retornar[1] === 'Noviembre' && retornar[2] === 'Enero'){
  return retornar;
